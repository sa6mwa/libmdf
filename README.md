...
The shared library uses SONAME ABI version `1`. This release intentionally
bumps the ABI because `mdf_options` gained public deck and chart controls.
...
falls back to `mdf`, and can be overridden with `--title` or `-T`. Generated
HTML documents include the libmdf provenance comment shown in the API section.
